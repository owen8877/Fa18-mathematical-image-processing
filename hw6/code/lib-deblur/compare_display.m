function compare_display(options)
    ...
    figure(15); clf
    if display_truth
        t_a = helper(truth_image, details_coor, [], [0 0 1/3]);
        axes(t_a.p_a)
    ...
    if ~default(options, 'no_truth', false)
        b_a = helper(blurred_image, details_coor, kernel_image, [1/3 0 1/3]);
        axes(b_a.p_a)
...
%         title(sprintf('(%dx%d)', size(kernel_image, 1), size(kernel_image, 2)))
        ...
        b_a = helper(blurred_image, details_coor, [], [1/3 0 1/3]);
        axes(b_a.p_a)
        title('Blurred image')
    end
    if display_recover
        r_a = helper(result_image, details_coor, deblur_kernel_image, [2/3 0 1/3]);
%         axes(r_a.k_a)
%         title(sprintf('(%dx%d)', size(deblur_kernel_image, 1), size(deblur_kernel_image, 2)))
    ...
    set(gcf, 'Position', [43 309 1064*1.5 232*1.5])
    I = getframe(gcf); imwrite(I.cdata, sprintf('image/compare/%s.jpg', p1name), 'Quality', 100); 
end
...
function cc = c(coor, si)
    ...
    cc = [coor(1)*s+xo coor(2)+yo coor(3)*s coor(4)];
end
