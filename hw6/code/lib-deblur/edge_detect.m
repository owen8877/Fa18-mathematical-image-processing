function edge_detect(options)
    addpath lib-mat lib-util lib
    p1name = options.p1name;

    f_blurred = double(imread(sprintf('image/blurred/%s.png', p1name)));

    % dark channel attempt; obselete
    % avg_size = 7;
    ...
    soptions = struct();
    soptions.pde_dt = default(options, 'sf_pde_dt', 2e-3);
    soptions.pde_maxTime = default(options, 'sf_pde_maxTime', 0.0);
    if soptions.pde_maxTime > 1e-16
        switch default(options, 'sf_pde_processor', 'sf')
            case 'sf'
                processor = @shockfilter;
            case 'pm'
                processor = @pmpde;
        end
        f_shock = processor(f_blurred, soptions);
        imshow(f_shock ./ 255)
    else
        f_shock = f_blurred;
    end

    % obselete blur attempt
    % blur_kernel = fspecial('gaussian', 11, 0.5);
    % f_shock_blurred = imfilter(f_shock, blur_kernel);
    f_shock_blurred = f_shock;

    %% Then construct patch candidate list
    ...
    box_size = options.edge_box_size;
    
    if ~default(options, 'no_truth', false)
        kernel_path = sprintf('image/kernel/%s.bmp', options.kernel_name);
        known_kernel = normalizeFirst(double(imread(kernel_path)));
        known_kernel = padarray(known_kernel, [1 1] * (box_size-(size(known_kernel, 1)-1)/2), 0, 'both');
    else
        known_kernel = zeros(2*box_size+1);
    end

    offset = floor(prod(g_shape)*options.edge_offset);
    maxN = floor(prod(g_shape)*options.edge_max_percent);
    ...
    plotOn = default(options, 'edge_plot', false);
    ...
    %     fprintf('Loss is %.3e.\n', loss)
        ...
        if lossness > options.edge_lossness
            continue
        end
    ...
    f_anno_dis = f_anno./max(max(f_anno));
    imshow(f_anno_dis, [])
    
    gradient_information_path = sprintf('image/intermediate/%s.jpg', p1name);
    imwrite(uint8(f_anno_dis*256), gradient_information_path, 'jpg', 'Quality', 100)

    edge_info_path = sprintf('data/%s.mat', options.edge_data);
    save(edge_info_path, 'information', 'counter', 'box_size', 'xp')
end
% edge_box_size
% edge_data
% edge_lossness
